...
export function WirthMarkovFSM(buffer, buffer_pointer, itable){
    let lines = [];
    let line = [];
	...
	if(buffer[index] != 0x22 && buffer[index]!= 0xA && buffer[index] !=0x2C){
	    ...
	    let [in_progress, cell , new_index] = parseUnescaped(buffer, index );
	    line.push(cell);
	    index = new_index;
...
function parseUnescaped(buffer, index){
	...
	if(buffer[index]==0x2C || buffer[index]==0x0A)
	    return [true, buffer.subarray(start, index), index ];
    }
