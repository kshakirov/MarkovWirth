...
export function WirthMarkovFSM(buffer, bufferIndex, itable, itableIndex, parserState){
   ...
   // console.log(`parserState:[${parserState}]bufferIndex is ${bufferIndex}`)
	...
	console.log("Restarting  escaped");
	let [status, nextIndex,,nextItableIndex ] = MarkovStep(index,buffer, itable, itableIndex);
	console.log(status, nextIndex, index);
	if(status){
	    console.log("Gere")
	    return [status, nextIndex, itable, nextItableIndex];
	}
	index = nextIndex;
	itableIndex = nextItableIndex;
    }
...
function MarkovStep(index, buffer,itable, itableIndex){

    let [finished, start, end, needsMoreInput] = MarkovEngine(index + 1,buffer)
		...
		//		line.push(cell);
		return [false, index, itable, itableIndex];

	    }else{
		//console.log(`Error old index is ${index}`);
		if(needsMoreInput){
		    //console.log("Capacity exhausted") ;
		    return [ESCAPED, start, itable, itableIndex ] //which start here not sure
		}
		else if(end === buffer.length - 2 && buffer[buffer.length - 1] === 0x22){
		   //if buffer finishes with " just a quote this is valid for casess where there is no new line and is supposed to be EOF file but this one is external to Wirth
		    ...
		    throw(`Escaped Cell Error: start: [${start}], end: [${end}], needsMoreInput: [${needsMoreInput}] `)//needs to be specified
		}
	    }
