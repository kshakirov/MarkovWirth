...
export function MarkovEngine(index, buffer) {
    ...
    outer_loop: do {
        ...
        if (matchResult) {
            ...
            if (r[0].length === 4) {
                if (r[0][3] === 0x2C) {
                    return [true, start, index + offset + 4];
                } else {
                    return [true, start, index + offset + 3];
                }
            } else if (r[0].length === 1) {
		const quoteAtBufferEnd = index + offset === buffer.length - 1;
                return [false, start, offset, quoteAtBufferEnd];
            } else if (r[1][0] === 0x03) {
                return [true, start, index + offset + 1];
            } else {
    ...
    return [false, start,capacity, true];
}
