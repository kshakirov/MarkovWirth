...
export function MarkovEngine(index, buffer) {
    ...
    outer_loop: do {
        ...
        if (matchResult) {
            ...
            if (r[0].length === 4) {
                if (r[0][3] === 0x2C) {
                    return [true, index + offset + 4, buffer.subarray(index - 1, index + offset + 4), start];
                } else {
                    return [true, index + offset + 3, buffer.subarray(index - 1, index + offset + 3), start];
                }
            } else if (r[0].length === 1) {
                return [false, offset, buffer.subarray(index - 1, offset, start)];
            } else if (r[1][0] === 0x03) {
                return [true, index + offset + 1, buffer.subarray(index - 1, index + offset + 1), start];
            } else {
    ...
    return [false, capacity, tmp, start];
}
